...
export const useMobileDragDrop = (onDrop, columnStatus) => {
  ...
  const handleTouchStart = useCallback((e, card) => {
    ...
    cardElement.style.transform = `translate(${touch.clientX - dragOffset.current.x}px, ${touch.clientY - dragOffset.current.y}px) scale(1.05)`;
  ...
  const handleTouchMove = useCallback((e) => {
    ...
    const columnElement = elementsAtPoint.find(el => 
      el.classList.contains('main-column') || 
      el.closest('.main-column')
    );
  ...
  const handleTouchEnd = useCallback(async (e) => {
    ...
    if (columnElement && onDrop) {
      ...
      const newStatus = column.querySelector('.column-title')?.textContent?.split(' (')[0];
